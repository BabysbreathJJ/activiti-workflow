...
LPB.plugins['text'] = function (active_component,leipiplugins) {
  ...
  $(popover).find("#orgvalue").val($(leipiplugins).text());
  $(popover).find("#orgdataname").val($(leipiplugins).attr("name"));
  //右弹form  取消控件
...
//          alert(attr_name);
          switch(attr_name)
          {
            ...
            case 'orgname':
              ...
              break;
            case 'orgdataname':
            	$(leipiplugins).attr("name",attr_val);
            	break;
            default:
...
e 是 class="leipiplugins" 的控件
*/
LPB.plugins['text_input'] = function (active_component,leipiplugins) {
  var plugins = 'text_input',popover = $(".popover");
  //右弹form  初始化值
  $(popover).find("#orgname").val($(leipiplugins).attr("title"));
              ...
              $.each(checkboxes, function(i,e){
                if(e.length > 0){
                 ...
                 if(vName) cName = vName;
                  if(!vName) vName = cName; if(!vTitle) vTitle = ''; if(!orginline) orginline = '';
